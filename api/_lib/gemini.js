...
조건:
- 난이도는 고3 수험생이 매일 꾸준히 풀며 실력을 쌓기에 적당한 수준으로 하세요. 실제 대학 정시 논술 최상위권 문제만큼 극도로 꼬거나 전제를 여러 겹 숨기지 말고, 개념을 정확히 이해하고 두 제시문의 입장 차이를 스스로 파악하면 풀 수 있는 정도로 설계하세요. 완전히 뻔한 이분법은 피하되, 지나치게 난해한 함정은 넣지 마세요.
...
조건:
- 지문(label: "지문")은 1000~1200자 분량으로, 하나의 명확한 핵심 주장 또는 쟁점을 담은 설명문·논설문 형태로 작성하세요. 고등학교 교육과정 과목(통합사회, 생활과 윤리, 윤리와 사상, 사회·문화, 경제, 세계사, 세계지리, 통합과학, 문학, 국어 등) 중 하나와 자연스럽게 연결되는 소재를 쓰세요. 문단을 2~4개로 구성해 논지가 전개되는 흐름이 드러나게 하세요.
- <보기>(label: "보기")는 300~350자 분량으로, 지문을 해석하거나 적용하는 데 필요한 추가 자료(다른 관점, 구체적 사례, 관련 개념 설명, 통계나 일화 등)를 담으세요. 지문과 직접 연결되어야 합니다.
