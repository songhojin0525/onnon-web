...
async function generateOneProblem(skill, dateStr) {
  const system = '당신은 성균관대학교·고려대학교 인문계열 논술 출제위원입니다. 실제 대학 정시/수시 논술고사 수준의 변별력 있는 문제를 출제합니다. 두 학교의 실제 기출 논술문제 형식(고등학교 교육과정 과목과 자연스럽게 연계되는 소재, 설명문뿐 아니라 시·소설 등 문학 제시문도 활용하는 구성, 두 입장을 대비시키는 압축적인 문항 형태)을 참고해, 특정 대학의 실제 지문이나 문항 문장을 베끼지 않고 완전히 새로운 문제를 창작합니다. 응답은 오직 하나의 JSON 객체여야 하며, 그 외의 설명이나 마크다운 코드블록 표시는 절대 포함하지 마세요.';
...
- ${skill.id === 'chart' ? `chart 필드를 반드시 포함하세요. 다음을 엄격히 지키세요:
  ...
  3) unit은 반드시 표기하고, title은 수치 내용과 정확히 일치해야 합니다.
  4) 만든 뒤 categories, series 길이와 unit 표기를 스스로 다시 확인하고 최종 출력하세요.` : '문제 성격상 자연스러우면 chart를 포함하고, 아니면 null로 두세요.'}
